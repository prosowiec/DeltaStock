...
def load_initialize_tickers(tickers, spy_loaded = False, batch = 16):
    sparkClass = sparkDelta()
    if not spy_loaded:
        initialize_spy_ticker_sec(sparkClass)
    load_ticket_to_delta(sparkClass, batchSize=batch, tickers = tickers)
